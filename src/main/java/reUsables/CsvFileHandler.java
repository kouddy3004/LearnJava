...
public class CsvFileHandler {
    ...
    public static CsvFileHandler on(){return new CsvFileHandler();}
    ...
    public String compareCsv(String sourcePath, String targetPath,String outputPath) {
        ...
        try {
                ...
                for (int row = 0; row < sourceList.size(); row++) {
                    ...
                    for (int column = 0; column < sourceList.get(0).length; column++) {
                        if (!sourceList.get(row)[column].equals(targetList.get(row)[column])) {
                            ...
                            errSize = errSize + 1;
                            MasterDriver.test.log(LogStatus.FAIL, "Source : " + sourceList.get(0)[column] + " -- > " + sourceList.get(row)[column]);
                            MasterDriver.test.log(LogStatus.FAIL, "Target : " + targetList.get(0)[column] + " -- > " + sourceList.get(row)[column]);
                            checkStatus[1] = "FAIL";
                        } else {
                            if (row > 0) {
                                MasterDriver.test.log(LogStatus.PASS, "Source : " + sourceList.get(0)[column] + " -- > " + sourceList.get(row)[column]);
                                MasterDriver.test.log(LogStatus.PASS, "Target : " + targetList.get(0)[column] + " -- > " + sourceList.get(row)[column]);
                            }
                        }
            ...
            if(checkStatus[1].equalsIgnoreCase("fail")) {
                ...
                createCsv(outputPath + "/" + MasterDriver.testData.get("TestCaseID") + ".csv", Lists.reverse(errorList),true);
                status="FAIL";
            }
